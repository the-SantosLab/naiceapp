...
class ServiceManager: ObservableObject {
    ...
    func checkStatus() { contactsAuthorized = CNContactStore.authorizationStatus(for: .contacts) == .authorized || CNContactStore.authorizationStatus(for: .contacts) == .limited; locationAuthorized = (locationManager.authorizationStatus == .authorizedWhenInUse || locationManager.authorizationStatus == .authorizedAlways); remindersAuthorized = EKEventStore.authorizationStatus(for: .reminder) == .authorized }
    func requestAll() async {
        if CNContactStore.authorizationStatus(for: .contacts) == .notDetermined { do { contactsAuthorized = try await contactStore.requestAccess(for: .contacts) } catch { contactsAuthorized = false } } else { contactsAuthorized = CNContactStore.authorizationStatus(for: .contacts) == .authorized || CNContactStore.authorizationStatus(for: .contacts) == .limited }
